...
<<<<<<< HEAD
| `max30102.c` / `max30102.h` | MAX30102 驱动（已升级，参见[库升级说明](#库升级说明)） |
=======
| `max30102.c` / `max30102.h` | MAX30102 驱动 |
>>>>>>> 4b71fd291140a6f0ac21770e536e0be4632091bc
...
MAX30102 驱动已从原始版本升级。与旧版的主要差异：

| 变更项 | 旧版 | 新版 |
|---|---|---|
| `max30102_setup` 返回值 | `void`（静默失败） | **`bool`** — 每步 I2C 写入均检查返回值 |
| `max30102_reset` | `sleep_ms(500)` 盲等 | 轮询 MODE_CONFIG 寄存器直至完成（≤10ms） |
| `max30102_check` 指针读取 | 两次独立 `read_reg` 调用 | **单次 3 字节突发读取**（减少一次 I2C 事务） |
| `max30102_check` 返回值 | 原始样本间隙 | **实际解码样本数** |
| `max30102_read_temperature` | 直接读寄存器（可能是旧值） | **触发新转换**，轮询等待完成后读取 |
| `max30102_read_temperature_fixed` | 截断 `*625/100` | **四舍五入** `(*625+50)/100` |
| FIFO 环形缓冲区 | `if(n<0) n+=32` + `%=` | **`& FIFO_MASK` 位掩码**（无分支） |
| FIFO RED/IR 字节序 | **互换**（IR 在前，RED 在后） | **正确**（RED 在前，与数据手册一致） |
...
| 问题 | 可能原因 | 解决方法 |
|---|---|---|
| OLED 无显示 | I2C1 波特率过高 | 确认 `SSD1306_I2C_BAUD=100000` 在 CMakeLists.txt 中定义（非仅 MAX30102.c） |
